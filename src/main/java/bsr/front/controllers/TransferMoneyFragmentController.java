...
/**
 * Controller for transfer money fragment
 */
public class TransferMoneyFragmentController {
    @FXML
    ...
    public void transferAction(ActionEvent event) {
        ...
        String targetAccountNumber = targetTextField.getText();

        try {
            Operation newOperation = null;
            newOperation = ServerConnection.getInstance().getAccountService().transferMoney(title, amount, selectedBankAccountNumber, targetAccountNumber);
            DialogsUtil.showSuccess("Transfer succeeded \n Account balance: " + (double) newOperation.getBalanceAfter()/100 + "$");
            clearFormAndRefresh();
        } catch (AccountServiceException_Exception | NotValidException_Exception | OperationException_Exception | SessionException_Exception | AccountException_Exception | UserException_Exception | IOException_Exception  e) {
